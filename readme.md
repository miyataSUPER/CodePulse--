...
### ファイル情報取得の出力例（ファイルが存在する場合）

```
=== ファイル情報取得デモ ===
ファイル名: sample.txt      => テキスト, サイズ: 45 bytes, トークン数: 8
ファイル名: script.py       => スクリプト, サイズ: 156 bytes, トークン数: 12
ファイル名: photo.jpg       => 画像, サイズ: 2048576 bytes, トークン数: 12345
ファイル名: music.mp3       => 音声, サイズ: 3145728 bytes, トークン数: 9876
ファイル名: document.pdf    => ドキュメント, サイズ: 1024000 bytes, トークン数: 5432
```

**注意:** バイナリファイル（画像、音声、動画など）でもトークン数が計算されますが、意味のない値が返されます。
